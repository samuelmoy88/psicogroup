<x-admin-layout>
    <form action="{{ route('rating-feedback.update', $ratingFeedback->id) }}" method="post">
        @csrf
        @method('PUT')
        <div class="form-card">
            ...
            <div class="lg:flex lg:space-x-2">
                <div class="mb-4 text-sm w-full lg:w-1/2">
                    ...
                    <x-input type="text" value="{{ $ratingFeedback->title }}" id="title" name="title"/>
                </div>
                <div class="mb-4 text-sm w-full lg:w-1/2">
                    ...
                    <select name="type" id="type"
                        ...
                        @foreach($types as $type => $label)
                            <option value="{{ $type }}"
                            {{ $ratingFeedback->$type === $type ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
