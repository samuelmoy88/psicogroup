<x-admin-layout>
    <form action="{{ route('uneasiness.update', $unease->id) }}" method="post">
        @csrf
        @method('PUT')
        <div class="form-card">
            <div class="flex flex-wrap justify-between">
                <h2 class="font-bold text-xl mb-4">{{ __('uneasiness.edit_unease') }}</h2>
            ...
            <div class="mb-4 text-sm w-full">
                ...
                <x-input type="text" value="{{ $unease->title }}" id="title" name="title"/>
            </div>
        </div>
